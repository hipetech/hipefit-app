...
interface PersistedSeedDocument<T extends { slug: string }> {
  slug: string;
  data: Omit<T, 'slug'>;
}

type WriteAssertion = (value: unknown) => void;
...
function transformSeedDocument<T extends { slug: string }>(
  seed: T,
  collection: string,
  assertWrite: WriteAssertion
): PersistedSeedDocument<T> {
  ...
  try {
    ...
    throw new Error(
      `${collection}/${slug} failed persisted schema validation.${reason}`
    );
  }
  return { slug, data };
}
...
export async function seedExercises(
  ...
  const categoryDocuments = exerciseCategories.map((category) =>
    transformSeedDocument(
      category,
      'exerciseCategories',
      assertExerciseCategoryWrite
    )
  );
  const equipmentDocuments = equipment.map((item) =>
    transformSeedDocument(item, 'equipment', assertEquipmentWrite)
  );
  const exerciseDocuments = exercises.map((exercise) =>
    transformSeedDocument(exercise, 'exercises', assertExerciseWrite)
  );

  console.log(`\nSeeding global exercise library`);
  console.log(`   Environment: ${opts.env}`);
  console.log(`   Categories: ${categoryDocuments.length}`);
  console.log(`   Equipment:  ${equipmentDocuments.length}`);
  console.log(`   Exercises: ${exerciseDocuments.length}`);
  ...
  if (opts.dryRun) {
    ...
    for (const [collection, items] of [
      ['exerciseCategories', categoryDocuments],
      ['equipment', equipmentDocuments],
      ['exercises', exerciseDocuments],
    ] as const) {
  ...
  await chunkedBatch(
    db,
    categoryDocuments,
    (db, document) => db.collection('exerciseCategories').doc(document.slug),
    ({ data }) => data
  );
  await chunkedBatch(
    db,
    equipmentDocuments,
    (db, document) => db.collection('equipment').doc(document.slug),
    ({ data }) => data
  );
  await chunkedBatch(
    db,
    exerciseDocuments,
    (db, document) => db.collection('exercises').doc(document.slug),
    ({ data }) => data
  );
