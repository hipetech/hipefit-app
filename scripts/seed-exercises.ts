/**
 * Seed Firestore with global exercise groups and exercises.
 *
 * Prerequisites:
...
interface ExerciseSeed {
  ...
  groupKey: string; // matches the exerciseGroups doc ID
  equipment: string[];
...
const exerciseGroups: Record<string, ExerciseGroupSeed> = {
...
async function main() {
  console.log(`\n🏋️ Hipefit Exercise Seeder`);
  console.log(`   Groups: ${Object.keys(exerciseGroups).length}`);
  console.log(`   Exercises: ${exercises.length}`);
  if (DRY_RUN) console.log('   Mode: DRY RUN\n');
  else console.log('');

  // Dry run doesn't need Firebase connection
  if (DRY_RUN) {
  ...
  for (const [id, group] of Object.entries(exerciseGroups)) {
    const ref = db.collection('exerciseGroups').doc(id);
    batch1.set(ref, group);
  }

  await batch1.commit();
  console.log(`   ✅ ${Object.keys(exerciseGroups).length} groups written.\n`);

  // Seed exercises (batch limit is 500, we're well under)
  console.log('💪 Seeding exercises...');
  const batch2 = db.batch();

  for (const exercise of exercises) {
    const ref = db.collection('exercises').doc(); // auto-ID
    batch2.set(ref, {
      ...exercise,
