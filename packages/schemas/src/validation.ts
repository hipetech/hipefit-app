...
import type { BodyMeasurement, UserProfile, UserSettings } from './user';
import type {
  TemplateExercise,
  TemplateSet,
  Workout,
...
const TIMESTAMP_NANOSECONDS_PER_SECOND = 1_000_000_000;
...
function assertTimestamp(
  ...
  if (
    ...
    value.seconds < TIMESTAMP_MIN_SECONDS ||
    value.seconds > TIMESTAMP_MAX_SECONDS ||
    !('nanoseconds' in value) ||
    typeof value.nanoseconds !== 'number' ||
    !Number.isInteger(value.nanoseconds) ||
    value.nanoseconds < 0 ||
    value.nanoseconds >= TIMESTAMP_NANOSECONDS_PER_SECOND ||
    !('toDate' in value) ||
